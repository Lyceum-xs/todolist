...
### 5.SonarQube 代码质量检测
然有一些警告（如：维护性问题、测试覆盖率低、安全热点），但质量门总体通过，说明没有阻碍代码发布的重大问题。
各项指标分析（以最后一张图为准）
1. Security（安全）
	•	Open issues: 0（等级 A）
	•	没有已识别的安全漏洞，表现很好。
⸻
2. Security Hotspots: 3（等级 E）
...
1. Duplications（代码重复）
	•	0.0%（等级 A）
	•	没有检测到重复代码，结构性很好。
![alt text](4fb96f04aeba8bdb683e561d93233c3c.png)
![alt text](a4c5caf6ea6a1196d812367973ade1ed.png)
通过测试单元的修改，覆盖率增加
![alt text](4fb96ccab54825326a6e1390ca310c60.png)
